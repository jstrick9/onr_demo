...
>
> *Predictive:* three complementary models, all in gold, all on the **same ingested portfolio**. `ols_fy_v1` is an ordinary-least-squares fit of `total_funding ~ fiscal_year` per program area — two-year horizon, 95% residual band, plus **trend IDs** `TREND-ACCEL` / `TREND-STEADY` / `TREND-DECLINE`. Notebook `04` trains a Random Forest (large-award ≥ $1M) → Fund / Review / Defer, registered as `onr_demo.gold.grant_large_award`. Notebook `04b` trains an **IsolationForest** anomaly detector on grant + ERP features (budget spike, execution collapse, low-return concentration), registers `onr_demo.gold.funding_anomaly_detector` @ `champion`, and writes the review queue to `gold.grant_anomaly_scores`. Leadership sees who to fund, which program is accelerating, and which awards look anomalous — not a black-box score.
>
