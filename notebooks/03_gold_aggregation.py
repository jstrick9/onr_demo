...
# Funding-anomaly features + heuristic flags (04b overwrites scores with IsolationForest)
spark.sql(f"""
...
WITH fin AS (
    ...
    FROM `{catalog}`.`silver`.financial WHERE _is_active = true GROUP BY grant_no
),
area_stats AS (
    SELECT program_area, fiscal_year,
           approx_percentile(amount_usd, 0.5) AS median_amt, AVG(amount_usd) AS avg_amt
    FROM `{catalog}`.`silver`.grants WHERE _is_active = true
    GROUP BY program_area, fiscal_year
),
prior AS (
    SELECT program_area, fiscal_year + 1 AS fiscal_year, avg_amt AS prior_avg FROM area_stats
),
base AS (
    SELECT g.grant_no, g.title, g.program_area, g.fiscal_year, g.amount_usd AS award_amount,
...
SELECT grant_no, title, program_area, fiscal_year, award_amount, awardee, org_unit,
       classification_band, execution_rate, yoy_growth_ratio, amount_vs_area_median,
       CASE WHEN execution_rate < 0.76 THEN 'execution_collapse'
            ...
            ELSE 'none' END AS anomaly_type,
       CASE WHEN execution_rate < 0.76 OR (award_amount >= 3000000 AND amount_vs_area_median >= 1.8)
              OR (award_amount >= 2500000 AND execution_rate < 0.85) THEN 1 ELSE 0 END AS is_known_anomaly,
       current_timestamp() AS _updated_at
FROM base
""")
existing_model = None
try:
    existing_model = spark.table(f"`{catalog}`.`gold`.grant_anomaly_scores").select("model_name").limit(1).collect()
    existing_model = existing_model[0][0] if existing_model else None
except Exception:
    existing_model = None
if existing_model and "iforest" in str(existing_model).lower():
    print("grant_anomaly_scores left in place (iforest)")
else:
    spark.sql(f"""
    CREATE OR REPLACE TABLE `{catalog}`.`gold`.grant_anomaly_scores AS
    SELECT grant_no, title, program_area, fiscal_year, award_amount AS amount_usd, awardee,
           execution_rate, yoy_growth_ratio, amount_vs_area_median,
           CASE anomaly_type
                WHEN 'execution_collapse' THEN 0.92
                WHEN 'budget_spike' THEN 0.88
                WHEN 'low_return_concentration' THEN 0.80
                ELSE 0.12 END AS anomaly_score,
           CAST(is_known_anomaly AS BOOLEAN) AS is_flagged,
           anomaly_type AS predicted_type, anomaly_type, is_known_anomaly,
           'heuristic_rules_v1' AS model_name, current_timestamp() AS scored_at
    FROM `{catalog}`.`gold`.funding_features
    """)
    print("grant_anomaly_scores heuristic", spark.table(f"`{catalog}`.`gold`.grant_anomaly_scores").count())
...
tables = [
    ...
    "funding_forecast", "program_trends", "funding_features", "grant_anomaly_scores",
]
